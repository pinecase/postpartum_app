...
const hoursAgoIso = (h) => new Date(Date.now() - h * HOURS).toISOString();

// 预警规则（简化的临床阈值，供月子中心护理参考，不替代医嘱）
// 与 server/src/alerts.js 保持同一套规则，运行于 Cloudflare D1
...
export async function computeAlerts(db) {
  ...
  for (const b of babies) {
    ...
    const v = await db
      .prepare(`SELECT * FROM baby_vitals WHERE baby_id = ? ORDER BY time DESC LIMIT 1`)
      .bind(b.id)
      .first();
    ...
    const stools = await db
      .prepare(`SELECT COUNT(*) AS c FROM baby_diapers WHERE baby_id = ? AND time >= ? AND type LIKE '%便%'`)
      .bind(b.id, hoursAgoIso(24))
      .first();
    if (stools.c === 0) {
  ...
  const mothers = (await db.prepare(`SELECT * FROM mothers WHERE status = '在住'`).all()).results;
